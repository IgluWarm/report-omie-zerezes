...
export default async function handler(
  ...
  const today = req.query.time
    ? new Date(parseInt(req.query.time as string))
    : new Date();
  ...
  const pedidos = await fetch(
    "https://app.omie.com.br/api/v1/produtos/cupomfiscalconsultar/",
    {
      ...
      body: JSON.stringify({
        call: "CuponsFiscais",
        app_key: "3007538325792",
        app_secret: "526cb77a0090156cb1610c666c9e0112",
        param: [
          {
            nPagina: 1,
            dDtEmissaoDe: `${today.getDate()}/${
              today.getMonth() + 1
            }/${today.getFullYear()}`,
            dDtEmissaoAte: `${today.getDate()}/${
              today.getMonth() + 1
            }/${today.getFullYear()}`,
          },
        ],
